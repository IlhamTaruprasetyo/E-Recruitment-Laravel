<footer class="relative bg-[#020602] border-t border-[#93F514]/30 text-gray-400 overflow-hidden">
    <!-- Glow element in footer #93F514 -->
    <div class="absolute bottom-0 left-1/2 -translate-x-1/2 w-3/4 h-32 bg-[#93F514]/10 blur-[120px] pointer-events-none"></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
                ...
                <div class="flex items-center gap-3 pt-2">
                    @if(isset($mainCompany) && $mainCompany->website)
                        <a href="{{ Str::startsWith($mainCompany->website, ['http://', 'https://']) ? $mainCompany->website : 'https://' . $mainCompany->website }}" 
                           ...
                           class="px-3 py-1.5 rounded-lg bg-[#050e05] border border-[#93F514]/40 flex items-center gap-2 text-xs font-semibold text-gray-300 hover:text-[#93F514] hover:border-[#93F514] transition footer-official-btn">
            ...
            <div>
                ...
                <ul class="space-y-2.5 text-sm">
                    <li><a href="{{ route('home') }}" class="hover:text-[#93F514] transition">Beranda</a></li>
                    <li><a href="{{ route('about') }}" class="hover:text-[#93F514] transition">Tentang Kami</a></li>
                    <li><a href="{{ route('jobs.index') }}" class="hover:text-[#93F514] transition">Cari Lowongan</a></li>
                    <li><a href="{{ route('register') }}" class="hover:text-[#93F514] transition">Daftar</a></li>
                    <li><a href="{{ route('login') }}" class="hover:text-[#93F514] transition">Masuk</a></li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-2.5 text-sm">
                    @forelse($footerDepartments ?? [] as $fDept)
                        <li>
                            <a href="{{ route('jobs.index', ['department_id' => $fDept->id]) }}" class="hover:text-[#93F514] transition">
                                {{ $fDept->name }}
                            </a>
                        </li>
                    @empty
                        <li><a href="{{ route('jobs.index') }}" class="hover:text-[#93F514] transition">Semua Lowongan</a></li>
                    @endforelse
                </ul>
            </div>
                ...
                <ul class="space-y-3 text-sm">
                    ...
                    @if(isset($mainCompany) && $mainCompany->email)
                        <li class="flex items-center gap-2.5">
                            ...
                            <a href="mailto:{{ $mainCompany->email }}" class="text-gray-300 hover:text-[#93F514] transition truncate max-w-[200px]" title="{{ $mainCompany->email }}">
                                {{ $mainCompany->email }}
                            </a>
                        </li>
                    @endif
                    @if(isset($mainCompany) && $mainCompany->phone)
                        <li class="flex items-center gap-2.5">
                            ...
                            <a href="{{ $mainCompany->whatsapp_url ?? 'https://wa.me/' . preg_replace('/[^0-9]/', '', $mainCompany->phone) }}" target="_blank" rel="noopener noreferrer" class="text-gray-300 hover:text-[#93F514] transition truncate max-w-[200px]" title="{{ $mainCompany->phone }}">
                                {{ $mainCompany->phone }}
                            </a>
                        </li>
                    @endif
        ...
        <div class="mt-12 pt-8 border-t border-[#93F514]/20 flex flex-col sm:flex-row items-center justify-between text-xs text-gray-500 gap-4">
