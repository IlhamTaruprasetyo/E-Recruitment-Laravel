        ...
        <div x-data="{
            ...
            <div x-show="viewMode === 'grid'" x-transition:enter="transition ease-out duration-200"
                ...
                @forelse($jobs as $job)
                    ...
                    @endphp
                    <div
                        class="job-card-item reveal-on-scroll group relative rounded-3xl bg-gradient-to-b from-[#061506] to-[#030803] border border-[#93F514]/30 hover:border-[#93F514] p-6 flex flex-col justify-between transition-all duration-300 hover:shadow-2xl hover:shadow-black/60 hover:-translate-y-1 h-full"
                ...
                @forelse($jobs as $job)
                    ...
                    <div
                        ...
                        <div class="flex items-start sm:items-center gap-4.5 flex-1 min-w-0">
                            <!-- Logo -->
                            @if ($job->company?->logo_url)
                                <div class="w-14 h-14 mr-5 rounded-2xl bg-white border border-[#93F514]/40 p-2 flex items-center justify-center shrink-0 overflow-hidden shadow-sm">
                                    <img src="{{ $job->company->logo_url }}" alt="{{ $job->company->name }}" class="w-full h-full object-contain">
                                </div>
                            @else
                                <div class="w-14 h-14 mr-5 rounded-2xl bg-white border border-[#93F514]/40 flex items-center justify-center text-gray-900 font-extrabold text-xl shrink-0 shadow-sm">
                                    {{ strtoupper(substr($job->company?->name ?? 'M', 0, 2)) }}
                                </div>
                            @endif
